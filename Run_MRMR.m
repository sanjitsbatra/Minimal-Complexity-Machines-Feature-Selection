function [ Metrics ] = Run_MRMR( xTrain,yTrain,xTest,yTest, NF )
% This function runs MRMR and produces the output metrics

    % Obtain features for MRMR
    ...
    mrmr_indices = feast('mrmr',NF, xTrain, yTrain); %% selecting the top features(as many as LMCM, using the jmi algorithm)
    
    model = svmtrain2(yTrain,xTrain(:,mrmr_indices),'-s 0 -t 0 -q');
    [pred_labels,acc,decision_values] = svmpredict(yTest,xTest(:,mrmr_indices),model,'-q');
