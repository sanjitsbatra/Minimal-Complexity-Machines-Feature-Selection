...
//co-ordinate coding: [Pai] -> p*-1 elements
	...
	double bestMI=0; //current best MI
    ...
    for(int p=1;p<=min(Pstar-1,dim);p++) { //loop through feature sets of increasing size
        ...
        int currentThread=0;
 		...
 		do  //generate all feature combination and score
            ...
            double maxBonus=2*Ne*(HC-bestMI); //maximum bonus
...
void updateBestPa(int * &best_Pa,int * Pa,int p){ //update the best feature set
